...
import { SHELF, WOOD, caseHeight, shelfFloorY } from '../config.js';
 ...
 *
 * Os UVs sao uma projecao planar por face, em coordenadas de MUNDO: U corre ao
 * longo da tangente mais comprida de cada quad (o comprimento da tabua, a
 * altura da lateral), V na outra, ambos divididos por WOOD.TILE_M. Como a
 * origem e a mesma para toda face, o veio continua entre o tampo e a borda
 * frontal de uma mesma tabua de graca.
 */

// --------------------------------------------------- construcao de caixas ---

const axisLen = (t) => Math.abs(t[0]) + Math.abs(t[1]) + Math.abs(t[2]);
const axisOf = (t) => (t[0] ? 0 : t[1] ? 1 : 2);

/** Empilha um quad (2 triangulos) definido por origem + dois vetores tangentes. */
function pushQuad(pos, nrm, uvs, idx, o, u, v, n) {
  ...
  // Veio (U da textura) ao longo do lado maior. As tangentes sao sempre
  // eixo-alinhadas, entao "eixo" e o indice da unica componente nao nula.
  const along = axisLen(u) >= axisLen(v) ? axisOf(u) : axisOf(v);
  const across = along === axisOf(u) ? axisOf(v) : axisOf(u);

  for (const c of corners) {
    pos.push(c[0], c[1], c[2]);
    nrm.push(n[0], n[1], n[2]);
    uvs.push(c[along] / WOOD.TILE_M, c[across] / WOOD.TILE_M);
  }
  idx.push(base, base + 1, base + 2, base, base + 2, base + 3);
}

/** Cuboide eixo-alinhado de [x0,y0,z0] a [x1,y1,z1]. 24 vertices, 12 triangulos. */
function pushBox(pos, nrm, uvs, idx, [x0, y0, z0], [x1, y1, z1]) {
  const dx = x1 - x0;
  const dy = y1 - y0;
  const dz = z1 - z0;

  pushQuad(pos, nrm, uvs, idx, [x1, y0, z1], [0, 0, -dz], [0, dy, 0], [1, 0, 0]); // +X
  pushQuad(pos, nrm, uvs, idx, [x0, y0, z0], [0, 0, dz], [0, dy, 0], [-1, 0, 0]); // -X
  pushQuad(pos, nrm, uvs, idx, [x0, y1, z1], [dx, 0, 0], [0, 0, -dz], [0, 1, 0]); // +Y
  pushQuad(pos, nrm, uvs, idx, [x0, y0, z0], [dx, 0, 0], [0, 0, dz], [0, -1, 0]); // -Y
  pushQuad(pos, nrm, uvs, idx, [x0, y0, z1], [dx, 0, 0], [0, dy, 0], [0, 0, 1]); // +Z
  pushQuad(pos, nrm, uvs, idx, [x0, y0, z0], [0, dy, 0], [dx, 0, 0], [0, 0, -1]); // -Z
}
...
function geometryFromBoxes(boxes) {
  ...
  const uvs = [];
  const idx = [];
  for (const [min, max] of boxes) pushBox(pos, nrm, uvs, idx, min, max);
 ...
 *
 * Os dois walnut usam o mesmo mapa de madeira; o rodape so recebe um `color`
 * mais escuro (razao walnutDark/walnut), e `color` e uniform, nao define de
 * shader — entao walnut, walnutDark, o painel de fundo (outro mapa, mesmo
 * material) e todos os livros (Lambert + map) sao UM programa.
 */
const materials = {
  walnut: new MeshLambertMaterial({ map: woodTexture('BOARD') }),
  walnutDark: new MeshLambertMaterial({
    map: woodTexture('BOARD'),
    // Sem o terceiro argumento, `setRGB` interpreta no espaco de trabalho do
    // three, que e Linear-sRGB — e a tint e uma razao de Kd lineares.
    color: new Color().setRGB(...WOOD.DARK_TINT),
  }),
  backPanel: new MeshLambertMaterial({ map: woodTexture('PANEL') }),
};
